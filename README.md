# Ansible

Ansible is known as a configuration management tool used to automate installations and configurations. This is a small documentation of what Ansible is in my definition and some of the problems I try to solve with it. For most of the examples I'll be using **Vagrant** as a tool for managing my Virtual Machines.

# Table of Contents

- [Basic Ansible Overview](https://github.com/sxcdennis/Ansible/blob/master/Basic.md)

- [Basic YMAL Syntax](https://github.com/sxcdennis/Ansible/blob/master/ymal.md)

- [Installing Ansible Automatically with Vagrant Boxes](https://github.com/sxcdennis/Ansible/blob/master/vagrant.md)

- [Creating inventory files](https://github.com/sxcdennis/Ansible/blob/master/inventory.md)

- [Creating playbooks](https://github.com/sxcdennis/Ansible/blob/master/playbooks.md)

- [Reusable Playbooks: Includes, Imports, and Roles](https://github.com/sxcdennis/Ansible/blob/master/roles.md)

- [Examples of solving issues](https://github.com/sxcdennis/Ansible/blob/master/issues.md)
